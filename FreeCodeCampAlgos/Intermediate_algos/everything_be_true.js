...
function truthCheck(collection, pre) {
    //tally to verify that pre is true in all objects
    let tally= 0
    //itarate thru each object
    for (let i = 0; i < collection.length; i++) {
        //if pre is present in each object, add to the tally
        if (collection[i][pre]) {
            tally++
        }
    }
    //if tally is true for every object checked, return true
